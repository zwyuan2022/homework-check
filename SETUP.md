...
基于 Web 的局域网作业检查系统，教师创建教室后，学生可通过浏览器加入并共享屏幕展示作业。首页自动根据访问地址识别角色（localhost → 教师，局域网 IP → 学生）。教师可自定义教室码，控制共享权限（一次仅一人）、可见范围（仅教师/全班可见），以及听到学生发言。
...
1. 浏览器打开 `http://localhost:3000`（本机 localhost 自动识别为教师）
2. 输入姓名 → 可选填自定义教室码（留空自动生成）→ 创建教室
3. 点击「📋 复制邀请信息」发送给学生
...
2. 遇到证书警告 → 点击「高级」→「继续前往」
3. 页面自动识别为学生 → 输入姓名和教室码 → 加入
4. 等待教师点击「🎤 允许共享」（按钮变绿后即可共享）
5. 点击「📺 共享屏幕」展示作业（首次需允许麦克风和屏幕录制权限）

### 教师操作

- 点击学生卡片 → 全屏放大查看（按 ESC 退出）
- 点击「🎤 允许共享」→ 授予该学生共享权限（同时只能一人共享，自动收回前一人权限）
...
### Q: 学生端按钮是灰色的，无法共享？
A: 教师需要先点击学生卡片上的「🎤 允许共享」按钮，学生端按钮才会变绿。同一时间只允许一名学生共享。

### Q: 学生取消共享后按钮又变灰了？
A: 教师的授权在学生取消共享后**不会自动收回**，按钮应保持绿色。如果变灰，请检查教师端是否误点了取消。
